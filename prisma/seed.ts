...
const categories =  ['Fast food', 'Vegetarian', 'Drink', 'Spicy', 'Salty', 'Sour'];

function seedFoods() {
    Promise.all(foods.map(food => prisma.food.create({ data: { name: food.name, about: food.about, description: food.description, image: food.image, price: food.price } })))
